...
func (m *defaultAdminModel) Insert(ctx context.Context, data *Admin) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Username, data.PasswordHash, data.Nickname, data.AvatarUrl)
	return ret, err
}
...
func (m *defaultAdminModel) Update(ctx context.Context, newData *Admin) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.Username, newData.PasswordHash, newData.Nickname, newData.AvatarUrl, newData.Id)
	return err
}
